...
class ApiService {
  ...
  Future<List<int>> fetchFavorie(int? id) async {
    ...
    final response = await http
        .get(Uri.parse('$baseUrl/favori/$id'), headers: <String, String>{
  ...
  Future<bool> updateRessource(int ressourceId, String titre,
      String description, int idCat, int idType, int idTag) async {
    final prefs = await SharedPreferences.getInstance();
    String? token = prefs.getString('userToken');
    Map<String, dynamic> user = jsonDecode(token!);
    String accessToken = user['accessToken'];
  ...
  //creation compte
  Future<bool> creerUtilisateurAvecRole(String nom, String prenom, String email,
      String password, String tel, Role role) async {
    final response = await http.post(
      Uri.parse('$baseUrl/utilisateur'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
